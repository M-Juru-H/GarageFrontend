...
const CustomerList = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch('https://garagebackend-f7ny.onrender.com/appointments');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm(translate("deleteConfirm"))) {
      try {
        const response = await fetch(`https://garagebackend-f7ny.onrender.com/appointments/appointments/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
  ...
  const handleUpdate = async (customer) => {
    try {
      const response = await fetch(`https://garagebackend-f7ny.onrender.com/appointments/appointments/${customer.id}`, {
        method: 'PUT',
        headers: {
